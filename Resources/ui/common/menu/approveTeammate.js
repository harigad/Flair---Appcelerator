...
exports.init = function(data,place,callBack){
	...
	submit.addEventListener("click",function(){
			 var dialog = Ti.UI.createAlertDialog({
    			...
    			message: 'APPROVE ' + data.name + "?"
   			 });
   			 
   			  dialog.addEventListener('click', function(e){
    			if (e.index === 0){
    				ApproveFromServer(true,data.rid,place.id,data.uid,function(){
    					callBack();
    					w.close();
    				});
    			}
	...
	cancel.addEventListener("click",function(){
		 var dialog = Ti.UI.createAlertDialog({
    			cancel: -1,
    			buttonNames: ['DENY', 'CANCEL'],
    			message: 'DENY ' + data.name + "?"
   			 });
   			 
   			  dialog.addEventListener('click', function(e){
    			if (e.index === 0){
    				ApproveFromServer(false,data.rid,place.id,data.uid,function(){
    					callBack();
    					w.close();
    				});
    			}
...
function ApproveFromServer(approve,rid,pid,uid,callBack){
		var url = "http://services.flair.me/search.php";
		var _dataStr = {};
		_dataStr.type = "approveTeammate";
		_dataStr.rid = rid;
		_dataStr.pid = pid;
		_dataStr.uid = uid;
		if(approve){
			_dataStr.approve = true;
		}
