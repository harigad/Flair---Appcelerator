...
function DPUnitsToPixels(ThePixels) {
...
exports.init = function(_type) {
	...
	if (!Ti.App.isIos) {
		main.width = DPUnitsToPixels(Ti.Platform.displayCaps.platformWidth);
	}
	...
	menu_a.add(Ti.UI.createLabel({
		borderRadius : 4,
		height : 50,
		width : 100,
		backgroundImage : "/images/home/home_checkin_btn.png"
	}));
	var menu_b = Ti.UI.createView({
		backgroundColor : "#66b5ff",
		opacity : 0.85,
		right : 0,
		height : 50,
...
function loadData(_type, _container, _date, _searchType, _searchWord) {
	if (!_container) {
		...
		var network_error = Ti.UI.createLabel({
			text : " ",
			color : "#cecece",
			font : {
				fontSize : 18
		...
		click_to_refresh = Ti.UI.createView({
			layout : "vertical",
			top : 150
		});
...
function header() {
	...
	var reload_btn = Ti.UI.createView({
		top : 25,
		bottom : 10,
		left : 20,
		right : 20,
	...
	var search_btn = Ti.UI.createView({
		opacity : 0.5,
		left : 0,
		height : 20,
		width : 20,
