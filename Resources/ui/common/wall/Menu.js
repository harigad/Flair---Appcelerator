...
exports.init = function(){
	...
	_main = Ti.UI.createWindow({
		backgroundColor:"#eee",navBarHidden:true,top:0,
	});	
	...
	nav = Titanium.UI.iOS.createNavigationWindow({
   		window: _main,width:Ti.Platform.displayCaps.platformWidth
	});
	nav.open();
...
function _draw(){
	...
	_view.add(item({closeMenu:true,approved:-1,icon:"images/menu_me_icon.png",title:"My flairs",callBack:function(){
		login.init(function(){
			user = login.getUser();
	...
	_view.add(item({closeMenu:false,approved:-1,icon:"images/menu_about_me_icon.png",title:"About Me",callBack:function(label){
	...
	if(false){
		_view.add(item({approved:-1,icon:"images/menu_invite_icon.png",title:"Invite",callBack:function(){
...
function item(data){
	...
	//}
		...
		var title = Ti.UI.createLabel({
			text:data.title,height:16,width:Ti.UI.SIZE,
			color:"#aaa",left:15,right:0,
				shadowColor: '#fff',
