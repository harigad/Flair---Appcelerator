...
//Rare exceptions to indicate a bug in the _flair code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_flair = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_flair]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.flair.module"
