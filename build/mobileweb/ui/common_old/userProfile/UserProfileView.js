...
function printDetails(_refresh){
	...
	if(user.getId()){
		...
		var feed = new FeedView(user.feed());	
		view.add(feed);
	}else{
...
function userHasNotClaimed(){
	...
	var one = Ti.UI.createLabel({
  		left:0,
  		right:0,
  		width:'100%',
  		color: '#999',  	
  		text: main.userName + " is yet to approve his/her Flairs!",
  			font: {
         		fontSize: 20
    		}
  	});
...
function printPlace(user){
	...
	var settingsText = Ti.UI.createLabel({
  	...
  	placeName.addEventListener('click',function(){
		...
		portal.open(placeView.init(place.pid,place.name));
  	});
